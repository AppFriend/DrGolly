...
export class KlaviyoService {
  ...
  /**
   * Track events to Klaviyo's Events API
   * @param eventName - Name of the event (e.g., "App_Purchase", "Abandoned_Cart")
   * @param userEmail - User's email address (profile identifier)
   * @param properties - Event properties object
   */
  async trackKlaviyoEvent(eventName: string, userEmail: string, properties: any = {}): Promise<boolean> {
    if (!KLAVIYO_API_KEY) {
    ...
    try {
      console.log(`Tracking Klaviyo event: ${eventName} for user: ${userEmail}`);
      ...
      if (!response.ok) {
        ...
        console.error(`Klaviyo event tracking failed: ${response.status} ${response.statusText}`, errorText);
        return false;
      }

      console.log(`✅ Klaviyo event ${eventName} tracked successfully for ${userEmail}`);
      return true;
    } catch (error) {
  ...
  /**
   * Track App_Purchase event when user completes a purchase
   */
  async trackAppPurchase(userEmail: string, purchaseData: {
    item_names: string[];
    item_ids: string[];
    total_value: number;
    currency: string;
    ...
    const properties = {
      item_names: purchaseData.item_names,
      item_ids: purchaseData.item_ids,
      total_value: purchaseData.total_value,
      currency: purchaseData.currency || "AUD",
      purchase_channel: "App",
      course_count: purchaseData.course_count,
      purchase_date: purchaseData.purchase_date || new Date().toISOString()
    };

    return await this.trackKlaviyoEvent("App_Purchase", userEmail, properties);
  }

  /**
   * Track Abandoned_Cart event when user abandons cart
   */
  async trackAbandonedCart(userEmail: string, cartData: {
    ...
    const properties = {
      ...
      cart_course_count: cartData.cart_course_count,
      abandoned_at: new Date().toISOString()
    };

    return await this.trackKlaviyoEvent("Abandoned_Cart", userEmail, properties);
  }
